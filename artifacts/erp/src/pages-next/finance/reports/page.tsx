...
export default function FinanceReportsPage() {
  ...
  return (
    <AppShell {...ACCOUNTING_CONFIG} navItems={ACCOUNTING_NAV} activeHref="/finance/reports">
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
        <div><h1 className="text-xl font-bold" style={{ color: '#1E1B4B' }}>Laporan Keuangan</h1><p className="text-sm mt-0.5" style={{ color: '#9CA3AF' }}>Ringkasan laba-rugi dan arus kas</p></div>
              ...
              <tbody>
                {MONTHLY.map((m, i) => (
                  <tr key={m.bulan} style={{ borderBottom: i < MONTHLY.length - 1 ? '1px solid #F5F2FB' : 'none' }}
                    ...
                    <td className="px-6 py-3.5 text-sm font-semibold" style={{ color: '#4CAF50' }}>{m.pendapatan}</td>
                    <td className="px-6 py-3.5 text-sm font-semibold" style={{ color: '#EA5455' }}>{m.pengeluaran}</td>
                    <td className="px-6 py-3.5 text-sm font-bold" style={{ color: '#2196F3' }}>{m.laba}</td>
                    <td className="px-6 py-3.5"><span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ backgroundColor: 'rgba(76,175,80,.1)', color: '#4CAF50' }}>{m.margin}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
