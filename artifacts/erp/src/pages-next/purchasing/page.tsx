...
import {
  FileText, ShoppingBag, Building2, DollarSign, TrendingUp, TrendingDown,
  Clock, ChevronRight, Plus, PackageCheck, AlertTriangle,
} from 'lucide-react';
...
export default function PurchasingPage() {
  ...
  return (
    <AppShell {...PURCHASING_CONFIG} navItems={PURCHASING_NAV} activeHref="/purchasing">
      <div className="p-6 space-y-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
          ...
          <div className="xl:col-span-3 rounded-2xl" style={{ backgroundColor: C.card, border: `1.5px solid ${C.border}`, boxShadow: '0 1px 4px rgba(47,43,61,.06)' }}>
            ...
            <div>
              {pendingPOs.map(({ id, supplier, amount, status, color }, i) => (
                <div key={id} className="flex items-center gap-3 px-5 py-3.5"
                  style={{ borderBottom: i < pendingPOs.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg flex-shrink-0"
                    style={{ backgroundColor: `${color}15` }}>
                    <ShoppingBag style={{ color, width: 15, height: 15 }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-semibold" style={{ color: C.heading }}>{id}</p>
                    <p className="text-[11px]" style={{ color: C.muted }}>{supplier}</p>
                  </div>
